...
import { Link, useNavigate } from 'react-router-dom'; // ✅ The Fix
...
const Home = () => {
  ...
  return (
    <div className={`transition-opacity duration-1000 bg-white ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
              ...
              {testimonials.map((testimonial, idx) => (
                <div key={idx} className="bg-white rounded-2xl shadow-xl shadow-slate-200/70 p-8 border border-slate-100">
                  ...
                  <div className="flex text-yellow-400 mb-5">
                    {[...Array(5)].map((_, i) => (
                      <IoStar key={i} className={i < testimonial.rating ? "text-yellow-40 wielding-the-power-of-the-sun" : "text-slate-300"} />
                    ))}
                  </div>
